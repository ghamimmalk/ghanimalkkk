      ...
      if (err) {
       ...
       // } catch (err) {util.embed("An error occured... \n**Console:**\n```js\n"+err+"```")}
        ...
        if (resourcee.length > 850) {
            var resourc = `الايبي  connect cfx.re/join/b9dz6d `
        } else {
        ...
        .addField("عدد الاعبين", `${e}/${start2.vars.sv_maxClients}`, true)
        .setThumbnail(icon)

     

        .addField("الايبي", "```  connect cfx.re/join/b9dz6d ```")
        
        //.setFooter(`Follow me instagram.com/that.ziv :)`, icon)
        .setFooter(`made by : ✰Ghanim✰♛#0932`, icon)
